...
"""
_MC_MODAL_HTML += (
    '\n<!-- BSN Stats Editor v3 -->\n<div id="bsn-stats-editor-v3" style="display:none;position:fixed;inset:0;z-index:9200;background:rgba(0,0,0,.93);backdrop-filter:blur(14px);overflow-y:auto;padding:20px 12px;-webkit-overflow-scrolling:touch">\n  <div style="max-width:460px;margin:0 auto;background:#0a0f1a;border-radius:22px;border:1px solid rgba(245,166,35,.4);padding:22px;box-shadow:0 40px 120px rgba(0,0,0,.95)">\n    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px">\n      <div>\n        <div style="font-size:.58rem;color:#f5a623;letter-spacing:2.5px;font-family:monospace">BSN &middot; TEAM STATS 2026</div>\n        <div style="font-size:1rem;font-weight:800;color:#f1f5f9;margin-top:3px">Editar Stats de Equipos</div>\n      </div>\n      <button onclick="closeBsnEditorV3()" style="background:rgba(255,255,255,.08);border:none;color:#94a3b8;font-size:1.1rem;padding:7px 14px;border-radius:10px;cursor:pointer">&#x2715;</button>\n    </div>\n    <div style="font-size:.63rem;color:#475569;margin-bottom:16px;padding:9px 12px;background:rgba(245,166,35,.05);border-radius:10px;border:1px solid rgba(245,166,35,.12)">\n      Pre-cargados desde el JSON. Edita los que cambiaron y guarda.\n    </div>\n    <div id="bsev3-cards" style="display:flex;flex-direction:column;gap:8px">\n      <div style="padding:30px;text-align:center;color:#475569;font-size:.8rem">Cargando...</div>\n    </div>\n    <button onclick="saveBsnEditorV3()" style="width:100%;margin-top:16px;padding:14px;font-size:.88rem;border-radius:14px;font-weight:800;background:linear-gradient(135deg,#f5a623,#e08c00);border:none;color:#000;cursor:pointer">&#x1F4BE; Guardar Cambios</button>\n    <div id="bsev3-msg" style="margin-top:10px;text-align:center;font-size:.8rem;color:#22c55e;min-height:18px"></div>\n  </div>\n</div>\n<style>\n#bsev3-cards .bse-tr{background:#111827;border-radius:12px;overflow:hidden}\n#bsev3-cards .bse-th{display:flex;align-items:center;justify-content:space-between;padding:9px 12px;font-size:.7rem;font-weight:700;color:#f1f5f9;border-bottom:1px solid rgba(255,255,255,.05)}\n#bsev3-cards .bse-nt{font-size:.65rem;font-family:monospace;font-weight:700}\n#bsev3-cards .bse-ri{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;padding:10px 12px}\n#bsev3-cards .bse-ri label{display:flex;flex-direction:column;gap:3px;font-size:.58rem;color:#475569;letter-spacing:.8px}\n#bsev3-cards input[type=number]{background:#1e293b;border:1px solid rgba(245,166,35,.15);border-radius:8px;color:#f1f5f9;font-size:.9rem;padding:7px 4px;width:100%;text-align:center;font-family:monospace;outline:none;box-sizing:border-box}\n#bsev3-cards input[type=number]:focus{border-color:#f5a623}\n</style>\n<script>\n(function(){\nvar _v3l=false,_v3d={};\nwindow.openBsnStatsEditor=function(){\n  var el=document.getElementById("bsn-stats-editor-v3");\n  if(!el){console.error("BSN editor element missing");return;}\n  el.style.display="block";document.body.style.overflow="hidden";\n  if(!_v3l)_v3Load();\n};\nwindow.closeBsnEditorV3=function(){\n  var el=document.getElementById("bsn-stats-editor-v3");\n  if(el){el.style.display="none";document.body.style.overflow="";}};\nwindow.closeBsnStatsEditor=window.closeBsnEditorV3;\nasync function _v3Load(){\n  var c=document.getElementById("bsev3-cards");\n  try{\n    var d=await(await fetch("/api/bsn/get-team-stats")).json();\n    _v3d=d.stats||{};var html="";\n    Object.keys(_v3d).sort().forEach(function(t){\n      var s=_v3d[t]||{};\n      var o=(s.ortg||100).toFixed(1),dr=(s.drtg||100).toFixed(1),p=(s.pace||78.5).toFixed(1);\n      var net=(s.ortg||100)-(s.drtg||100),ns=(net>=0?"+":"")+net.toFixed(1);\n      var nc=net>2?"#22c55e":net<-2?"#f43f5e":"#94a3b8";\\n      html+=\\\'<div class="bse-tr" data-t="\\\'+t+\\\'"><div class="bse-th"><span>\\\'+t+\\\'</span><span class="bse-nt" style="color:\\\'+nc+\\\'">\\\'+ns+\\\'</span></div><div class="bse-ri"><label>OFF RTG<input type="number" step="0.1" data-f="ortg" value="\\\'+o+\\\'"></label><label>DEF RTG<input type="number" step="0.1" data-f="drtg" value="\\\'+dr+\\\'"></label><label>PACE<input type="number" step="0.1" data-f="pace" value="\\\'+p+\\\'"></label></div></div>\\\';\n    });\n    c.innerHTML=html;_v3l=true;\n    c.querySelectorAll("input").forEach(function(inp){\n      inp.addEventListener("input",function(){\n        var row=this.closest("[data-t]");\n        var o=parseFloat(row.querySelector("[data-f=ortg]").value)||0;\n        var dr=parseFloat(row.querySelector("[data-f=drtg]").value)||0;\n        var net=o-dr,ns=(net>=0?"+":"")+net.toFixed(1);\n        var ne=row.querySelector(".bse-nt");\n        ne.textContent=ns;ne.style.color=net>2?"#22c55e":net<-2?"#f43f5e":"#94a3b8";});});\n  }catch(e){c.innerHTML=\'<div style="color:#f43f5e;padding:20px;text-align:center">Error: \'+e+\'</div>\';}\n}\nwindow.saveBsnEditorV3=async function(){\n  var msg=document.getElementById("bsev3-msg");\n  msg.textContent="Guardando...";msg.style.color="#f5a623";var payload={};\n  document.querySelectorAll("#bsev3-cards [data-t]").forEach(function(row){\n    var t=row.dataset.t;\n    payload[t]={ortg:parseFloat(row.querySelector("[data-f=ortg]").value)||0,\n      drtg:parseFloat(row.querySelector("[data-f=drtg]").value)||0,\n      pace:parseFloat(row.querySelector("[data-f=pace]").value)||0};});\n  try{\n    var d=await(await fetch("/api/bsn/update-team-stats",{method:"POST",\n      headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)})).json();\n    msg.textContent=d.msg;msg.style.color=d.ok?"#22c55e":"#f43f5e";\n    if(d.ok){_v3l=false;setTimeout(window.closeBsnEditorV3,1500);}}\n  catch(e){msg.textContent="Error: "+e;msg.style.color="#f43f5e";}\n};\n})();\n</script>'
)
