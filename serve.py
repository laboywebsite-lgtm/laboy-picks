  ...
  try {
    ...
    list.innerHTML = picks.map(function(p){
      ...
      var dlBtn = p.ready
        ...
        : '<span class="pg-btn pg-gen" title="Generando pick card...">⏳</span>';
...
<div class="filters">
  ...
  <span id="count"></span>
  <button class="btn-filter" style="margin-left:auto;background:#0f172a;border-color:#334155"
    onclick="regenJpgs()">🔄 Regen Fotos</button>
</div>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
            ...
            try:
                ...
                for _e in (_all_picks if isinstance(_all_picks, list) else []):
                    if str(_e.get("date", "")) != _dtl:
                        continue
                    _pid = _e.get("id", 0)
                    # Look for JPG generated by PIL (fast, no Playwright needed)
        ...
        if self.path.startswith("/api/gallery/latest-pick"):
            ...
            # Look for JPG generated by PIL (no Playwright needed)
        ...
        if self.path == "/api/regen-picks-jpg":
            ...
            for _lg_r, _log_r, _dir_r, _script_r, _pfx_r in _cfg:
                ...
                for _e_r in _entries:
                    ...
                    _existing = _rglob.glob(os.path.join(_dir_r, f"{_pfx_r} {_dt_r} #{_pid_r}*.jpg"))
                    if _existing:
                        continue  # already has JPG
                    # Run --export-log to generate HTML + JPG via PIL
                    def _regen_one(script, cwd, pid, lg):
                        try:
                            _run(["python3", script, "--export-log", str(pid)],
                                 cwd=cwd, timeout=60)
                        except Exception as _re_e:
                            print(f"  ⚠️  regen {lg} #{pid}: {_re_e}")
                    threading.Thread(
                        target=_regen_one,
                        args=(_script_r, _dir_r, _pid_r, _lg_r),
                        daemon=True
                    ).start()
