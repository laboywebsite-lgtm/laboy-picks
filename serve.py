...
<!-- ── BSN Stats Editor (global) ───────────────────────── -->
<div id="bsn-stats-editor-v3-static" style="display:none;position:fixed;inset:0;z-index:9100;background:rgba(0,0,0,.92);backdrop-filter:blur(12px);overflow-y:auto;padding:20px 12px;-webkit-overflow-scrolling:touch">
  <div style="max-width:460px;margin:0 auto;background:#0a0f1a;border-radius:22px;border:1px solid rgba(245,166,35,.35);padding:22px;box-shadow:0 40px 120px rgba(0,0,0,.9)">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px">
      ...
      <button onclick="closeBsnStatsEditor()" style="background:rgba(255,255,255,.08);border:none;color:#94a3b8;font-size:1.1rem;padding:7px 14px;border-radius:10px;cursor:pointer">&#x2715;</button>
    </div>
    <div style="font-size:.63rem;color:#475569;margin-bottom:16px;padding:9px 12px;background:rgba(245,166,35,.05);border-radius:10px;border:1px solid rgba(245,166,35,.12)">
      Valores pre-cargados desde el JSON. Edita los que cambiaron en realgm.com y guarda.
    </div>
    ...
    <button onclick="saveBsnStats()" style="width:100%;margin-top:16px;padding:14px;font-size:.88rem;border-radius:14px;font-weight:800;background:linear-gradient(135deg,#f5a623,#e08c00);border:none;color:#000;cursor:pointer">
      &#x1F4BE; Guardar Cambios
    </button>
    <div id="bse-save-msg" style="margin-top:10px;text-align:center;font-size:.8rem;color:#22c55e;min-height:18px"></div>
  </div>
</div>
<style>
.bse-team-row{background:#111827;border-radius:12px;overflow:hidden}
.bse-row-hdr{display:flex;align-items:center;justify-content:space-between;padding:9px 12px;font-size:.7rem;font-weight:700;color:#f1f5f9;border-bottom:1px solid rgba(255,255,255,.05)}
...
.bse-row-inputs label{display:flex;flex-direction:column;gap:3px;font-size:.58rem;color:#475569;letter-spacing:.8px}
.bse-num-inp{background:#1e293b;border:1px solid rgba(245,166,35,.15);border-radius:8px;color:#f1f5f9;font-size:.9rem;padding:7px 4px;width:100%;text-align:center;font-family:monospace;outline:none;box-sizing:border-box}
.bse-num-inp:focus{border-color:#f5a623}
</style>
<script>
var _bseLoaded=false,_bseData={};
function openBsnStatsEditor(){
  document.getElementById('bsn-stats-editor-v3-static').style.display='block';
  document.body.style.overflow='hidden';
  if(!_bseLoaded){_bseLoadCards();}
}
...
async function _bseLoadCards(){
  var c=document.getElementById('bse-cards-container');
  try{
    var r=await fetch('/api/bsn/get-team-stats');
    var d=await r.json();
    _bseData=d.stats||{};
    var teams=Object.keys(_bseData).sort(),html='';
    teams.forEach(function(t){
      ...
      html+='<div class="bse-team-row" data-team="'+t+'">'
        +'<div class="bse-row-hdr"><span>'+t+'</span>'
        +'<span class="bse-row-net" style="color:'+nc+'">'+ns+'</span></div>'
        +'<div class="bse-row-inputs">'
        +'<label>OFF RTG<input class="bse-num-inp" type="number" step="0.1" data-f="ortg" value="'+o+'"></label>'
        +'<label>DEF RTG<input class="bse-num-inp" type="number" step="0.1" data-f="drtg" value="'+dr+'"></label>'
        +'<label>PACE<input class="bse-num-inp" type="number" step="0.1" data-f="pace" value="'+p+'"></label>'
        +'</div></div>';
    });
    c.innerHTML=html;
    _bseLoaded=true;
    c.querySelectorAll('.bse-num-inp').forEach(function(inp){
      inp.addEventListener('input',function(){
        ...
        var ne=row.querySelector('.bse-row-net');
        ne.textContent=ns;ne.style.color=net>2?'#22c55e':net<-2?'#f43f5e':'#94a3b8';
      });
    });
  }catch(e){c.innerHTML='<div style="color:#f43f5e;padding:20px;text-align:center">Error cargando stats: '+e+'</div>';}
}
async function saveBsnStats(){
  ...
  try{
    var r=await fetch('/api/bsn/update-team-stats',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
    var d=await r.json();
