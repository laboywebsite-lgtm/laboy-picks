...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
            ...
            try:
                ...
                for _e in (_all_picks if isinstance(_all_picks, list) else []):
                    if str(_e.get("date", "")) != _dtl:
                        continue
                    _pid = _e.get("id", 0)
                    # Look for HTML pick card generated by --export-log (real design)
                    _hmatches = _pglob.glob(os.path.join(_league_dir, f"{_jpfx} {_dtl} #{_pid}*.html"))
                    _ready  = bool(_hmatches)
                    _hname  = os.path.basename(_hmatches[0]) if _ready else ""
        ...
        if self.path.startswith("/api/gallery/latest-pick"):
            ...
            # Look for HTML pick card (real design from --export-log)
            _pattern = os.path.join(_ldir, f"{_prefix} {_dt} #*.html")
